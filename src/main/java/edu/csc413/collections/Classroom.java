...
public class Classroom {
    ...
    public void dropStudent(int id) {
        ...
        if(enrolledIds.remove(id)){
            enrolledIds.add(waitlistIds.remove());
        }
    }

    public ArrayList<String> getEnrolledStudents() {
        // TODO: Implement. Return the names of all students that are enrolled in an ArrayList<Student>.
        ArrayList<String> namesEnrolled = new ArrayList<>();
        for (int id: enrolledIds){
            if(registeredStudents.get(id).getId() == id){
                namesEnrolled.add(registeredStudents.get(id).getName());
            }
        }
        return namesEnrolled;
    }
    ...
    public ArrayList<String> getWaitlistedStudents() {
        ...
        ArrayList<String> namesWaitingReversed = new ArrayList<>();
        Stack<String> namesWaiting = new Stack<>();
        for (int id: waitlistIds) {
            if (registeredStudents.get(id).getId() == id) {
                namesWaiting.push(registeredStudents.get(id).getName());
            }
        }
        while (!namesWaiting.isEmpty()) {
                namesWaitingReversed.add(namesWaiting.pop());
            }

        return namesWaitingReversed;
    }
